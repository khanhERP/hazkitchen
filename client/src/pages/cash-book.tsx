...
export default function CashBookPage({ onLogout }: CashBookPageProps) {
  ...
  const { data: purchaseReceipts = [] } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/purchase-receipts"],
    queryFn: async () => {
      try {
        const response = await fetch("https://api-pos.edpos.vn/api/purchase-receipts");
        if (!response.ok)
          throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
  ...
  const { data: incomeVouchers = [] } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/income-vouchers"],
    queryFn: async () => {
      try {
  ...
  const { data: expenseVouchers = [] } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/expense-vouchers"],
    queryFn: async () => {
      try {
