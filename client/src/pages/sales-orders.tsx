...
export default function SalesOrders() {
  ...
  useEffect(() => {
    ...
    const handleEInvoiceModalClosed = async (event: CustomEvent) => {
      ...
      queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/order-items"] });
      queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/invoices"] });

      // Force immediate refetch with fresh data
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] }),
        queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/order-items"] }),
        queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/invoices"] }),
        queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/tables"] }),
        queryClient.refetchQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] }),
        queryClient.refetchQueries({ queryKey: ["https://api-pos.edpos.vn/api/invoices"] }),
      ]);
  ...
  useEffect(() => {
    const handleNewOrder = () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] });
      queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/invoices"] });
      queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/transactions"] });
      queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders/date-range"] });
      queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/invoices/date-range"] });
    };
  ...
  const {
    ...
    queryKey: [
      "https://api-pos.edpos.vn/api/orders/list",
      startDate,
      endDate,
      customerSearch,
      orderNumberSearch,
    ...
    queryFn: async () => {
      try {
        ...
        const url = `https://api-pos.edpos.vn/api/orders/list?${params.toString()}`;
  ...
  const { data: tables = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiRequest("GET", "https://api-pos.edpos.vn/api/tables");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const {
    ...
    queryKey: ["https://api-pos.edpos.vn/api/order-items", selectedInvoice?.id],
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest(
          "GET",
          `https://api-pos.edpos.vn/api/order-items/${selectedInvoice.id}`,
        );
  ...
  const updateOrderMutation = useMutation({
    mutationFn: async (updatedOrder: Order) => {
      ...
      const response = await apiRequest(
        "PUT",
        `https://api-pos.edpos.vn/api/orders/${updatedOrder.id}`,
        updatePayload,
      );
    ...
    onSuccess: async (data, updatedOrder) => {
      ...
      queryClient.removeQueries({
        queryKey: ["https://api-pos.edpos.vn/api/order-items", updatedOrder.id],
      });
  ...
  const publishRequestMutation = useMutation({
    mutationFn: async (invoiceData: any) => {
      const response = await apiRequest(
        "POST",
        "https://api-pos.edpos.vn/api/einvoice/publish",
        invoiceData,
      );
      return response.json();
    },
      ...
      if (result.success && selectedInvoice) {
        try {
          ...
          const updateResponse = await apiRequest(
            "PUT",
            `https://api-pos.edpos.vn/api/orders/${selectedInvoice.id}`,
            updateData,
          );
          console.log(
          ...
          queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] });
          queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders/list"] });
          queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/order-items"] });
          queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/invoices"] });
          ...
          await Promise.all([
            queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] }),
            queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders/list"] }),
            queryClient.refetchQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] }),
            queryClient.refetchQueries({
              queryKey: [
                "https://api-pos.edpos.vn/api/orders/list",
  ...
  const cancelOrderMutation = useMutation({
    ...
    onSuccess: async (data, orderId) => {
      ...
      if (selectedInvoice?.tableId) {
        try {
          ...
          if (!otherActiveOrders || otherActiveOrders.length === 0) {
            ...
            await apiRequest(
              "PUT",
              `https://api-pos.edpos.vn/api/tables/${selectedInvoice.tableId}/status`,
              {
                status: "available",
              },
            );
      ...
      // Clear cache completely and force fresh fetch
      queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] });
      queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders/date-range"] });
      ...
      await Promise.all([
        ...
        queryClient.refetchQueries({
          queryKey: [
            "https://api-pos.edpos.vn/api/order-range",
            startDate,
            endDate,
            currentPage,
            itemsPerPage,
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      for (const item of itemsToDelete) {
        ...
        const response = await apiRequest(
          "DELETE",
          `https://api-pos.edpos.vn/api/order-items/${item.id}`,
        );
        if (!response.ok) {
      ...
      for (const item of itemsToCreate) {
        ...
        const response = await apiRequest(
          "POST",
          `https://api-pos.edpos.vn/api/order-items/${editableInvoice.id}`,
          payload,
        );
      ...
      queryClient.removeQueries({ queryKey: ["https://api-pos.edpos.vn/api/order-items"] });
      queryClient.removeQueries({
        queryKey: ["https://api-pos.edpos.vn/api/orders/date-range"],
      });

      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/orders"] }),
        queryClient.invalidateQueries({
          queryKey: ["https://api-pos.edpos.vn/api/order-items", editableInvoice.id],
        }),
        queryClient.invalidateQueries({
          queryKey: ["https://api-pos.edpos.vn/api/orders/date-range"],
        }),
  ...
  const handlePayment = async (order: Invoice) => {
    ...
    try {
      const updateResponse = await apiRequest(
        "PUT",
        `https://api-pos.edpos.vn/api/orders/${order.id}`,
        {
          paymentStatus: "paid",
          status: "paid",
  ...
  const handlePaymentComplete = (data: any) => {
    ...
    if (data.success && data.receipt) {
      ...
      setTimeout(() => {
        ...
        queryClient.invalidateQueries({
          queryKey: [
            "https://api-pos.edpos.vn/api/orders/date-range",
            startDate,
            endDate,
            currentPage,
            itemsPerPage,
  ...
  const handlePrintReceipt = async (order: any) => {
    try {
      ...
      const response = await apiRequest("GET", `https://api-pos.edpos.vn/api/order-items/${order.id}`);
