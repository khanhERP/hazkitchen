...
export default function KitchenDisplay() {
  ...
  const { data: pendingItems, refetch: refetchPendingItems } = useQuery<
    PendingOrderItem[]
  >({
    queryKey: ["https://api-pos.edpos.vn/api/order-items/pending"],
    queryFn: async () => {
      const response = await fetch("https://api-pos.edpos.vn/api/order-items/pending");
      if (!response.ok) {
  ...
  const { data: progressItems, refetch: refetchProgressItems } = useQuery<
    PendingOrderItem[]
  >({
    queryKey: ["https://api-pos.edpos.vn/api/order-items/progress"],
    queryFn: async () => {
      const response = await fetch("https://api-pos.edpos.vn/api/order-items/progress");
      if (!response.ok) {
  ...
  const { data: completedItems, refetch: refetchCompletedItems } = useQuery<
    PendingOrderItem[]
  >({
    queryKey: ["https://api-pos.edpos.vn/api/order-items/completed"],
    queryFn: async () => {
      const response = await fetch("https://api-pos.edpos.vn/api/order-items/completed");
      if (!response.ok) {
  ...
  const { data: products } = useQuery<Product[]>({
    ...
    queryFn: async () => {
      const response = await fetch(
        "https://api-pos.edpos.vn/api/products?limit=50000&includeInactive=false",
      );
      if (!response.ok) {
  ...
  const updateOrderStatus = async (orderId: number, status: string) => {
    try {
      const response = await fetch(`https://api-pos.edpos.vn/api/orders/${orderId}/kitchen-status`, {
        method: "PATCH",
  ...
  const updateOrderItemStatus = async (itemId: number, status: string) => {
    try {
      const response = await fetch(`https://api-pos.edpos.vn/api/order-items/${itemId}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status }),
      });
