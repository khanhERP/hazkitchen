...
import { Check, Clock, ChefHat, AlertCircle, ArrowRight, Undo2, Utensils } from "lucide-react";
...
export default function KitchenDisplay() {
  ...
  const [activeStatusTab, setActiveStatusTab] = useState<"pending" | "progress" | "completed">("pending");

  // Fetch pending order items
  const { data: pendingItems, refetch: refetchPendingItems } = useQuery<PendingOrderItem[]>({
  ...
  const { data: progressItems, refetch: refetchProgressItems } = useQuery<PendingOrderItem[]>({
  ...
  const { data: completedItems, refetch: refetchCompletedItems } = useQuery<PendingOrderItem[]>({
  ...
  // Fetch pending orders (keep for backward compatibility)
  const { data: orders, refetch } = useQuery<KitchenOrder[]>({
    queryKey: ["https://edpos-be.onrender.com/api/orders/kitchen"],
    queryFn: async () => {
      const response = await fetch("https://edpos-be.onrender.com/api/orders/kitchen");
      if (!response.ok) {
  ...
  const { data: products } = useQuery<Product[]>({
    ...
    queryFn: async () => {
      const response = await fetch("https://edpos-be.onrender.com/api/products?limit=50000&includeInactive=false");
      if (!response.ok) {
  ...
  const groupedByTable = (pendingItems || []).reduce((acc, item) => {
  ...
  const groupedByDish = (pendingItems || []).reduce((acc, item) => {
              ...
              <CardHeader className="bg-gradient-to-r from-yellow-50 to-yellow-100 border-b-2 border-yellow-200">
                <CardTitle className="flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-xl font-bold text-gray-800">{t("kitchen.pending")}</span>
                  ...
                  {/* View Mode Toggle */}
                  <Tabs value={viewMode} onValueChange={(value) => setViewMode(value as "table" | "dish")} className="w-auto">
                  ...
                  <div className="space-y-4">
                    {viewMode === "table" ? (
                      ...
                      Object.keys(groupedByTable).length > 0 ? (
                        Object.entries(groupedByTable).map(([tableNumber, items]) => (
                          <Card key={tableNumber} className="border-2 border-yellow-200 shadow-lg hover:shadow-xl transition-all duration-200 overflow-hidden">
                            ...
                            <CardContent className="space-y-2 pb-3 pt-3 bg-gradient-to-b from-white to-gray-50">
                              {items.map((item, idx) => (
                                <div key={item.id} className="flex items-center gap-3 bg-white p-3 rounded-lg border-2 border-gray-100 hover:border-yellow-300 transition-all duration-200 shadow-sm">
                                  <div className="flex-1 min-w-0">
                                    <div className="font-bold text-base text-gray-800 truncate">{item.productName || "Unknown"}</div>
                                    <div className="text-xs text-gray-500 flex items-center gap-2 mt-1">
                                      <span className="px-2 py-0.5 bg-gray-100 rounded">{item.orderNumber}</span>
                                      <span>{item.orderedAt ?
                                  ...
                                  {item.productImage && (
                                    <img src={item.productImage} alt={item.productName} className="w-14 h-14 object-cover rounded-lg shadow-md border-2 border-gray-200" />
                                  )}
                                  ...
                                  <Button
                                    ...
                                    onClick={() => updateOrderItemStatus(item.id, "progress")}
                                  >
                      ...
                      Object.keys(groupedByDish).length > 0 ? (
                        Object.entries(groupedByDish).map(([dishName, items]) => {
                          ...
                          return (
                            <Card key={dishName} className="border-yellow-300 shadow-md">
                              <CardHeader className="pb-3 bg-yellow-50">
                                <div className="flex justify-between items-start">
                                  <div className="flex-1">
                                    <div className="font-bold text-lg">{dishName}</div>
                              ...
                              <CardContent className="space-y-3 pb-3 pt-3">
                                {items.map((item, idx) => (
                                  <div key={item.id} className="flex items-center gap-3 bg-white p-3 rounded border">
                                    <div className="flex-1">
                                      <div className="font-semibold">Bàn {item.tableNumber || "N/A"}</div>
                                      <div className="text-sm text-gray-500">
                                        {item.orderNumber} - {item.orderedAt ?
                                          new Date(item.orderedAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) :
                                          ''}
                                      </div>
                                    </div>
              ...
              <CardHeader className="bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200">
                <CardTitle className="flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-xl font-bold text-gray-800">{t("kitchen.readyToServe")}</span>
                ...
                <ScrollArea className="h-[calc(100vh-300px)]">
                  <div className="space-y-4">
                    {viewMode === "table" ? (
                      ...
                      (() => {
                        const groupedByTable = (progressItems || []).reduce((acc, item) => {
                          const tableKey = item.tableNumber || "N/A";
                          if (!acc[tableKey]) {
                            acc[tableKey] = [];
                          }
                        ...
                        return Object.keys(groupedByTable).length > 0 ? (
                          Object.entries(groupedByTable).map(([tableNumber, items]) => (
                            <Card key={tableNumber} className="border-blue-300 shadow-md">
                              ...
                              <CardContent className="space-y-3 pb-3 pt-3">
                                {items.map((item) => (
                                  <div key={item.id} className="flex items-center gap-3 bg-white p-3 rounded border">
                                    <div className="flex-1">
                                      <div className="font-semibold text-base">{item.productName || "Unknown"}</div>
                                      <div className="text-sm text-gray-500">
                                        {item.orderNumber} - {item.orderedAt ?
                                          new Date(item.orderedAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) :
                                          ''}
                      ...
                      (() => {
                        const groupedByDish = (progressItems || []).reduce((acc, item) => {
                          const dishKey = item.productName || "Unknown";
                          if (!acc[dishKey]) {
                            acc[dishKey] = [];
                          }
                        ...
                        return Object.keys(groupedByDish).length > 0 ? (
                          Object.entries(groupedByDish).map(([dishName, items]) => {
                            const totalQuantity = items.reduce((sum, item) => sum + (item.quantity || 0), 0);
                            return (
                              <Card key={dishName} className="border-blue-300 shadow-md">
                                <CardHeader className="pb-3 bg-blue-50">
                                  <div className="flex justify-between items-start">
                                    <div className="flex-1">
                                      <div className="font-bold text-lg">{dishName}</div>
              ...
              <CardHeader className="bg-gradient-to-r from-green-50 to-green-100 border-b-2 border-green-200">
                <CardTitle className="flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-xl font-bold text-gray-800">{t("kitchen.completed")}</span>
                ...
                <ScrollArea className="h-[calc(100vh-300px)]">
                  <div className="space-y-4">
                    {viewMode === "table" ? (
                      ...
                      (() => {
                        const groupedByTable = (completedItems || []).reduce((acc, item) => {
                          const tableKey = item.tableNumber || "N/A";
                          if (!acc[tableKey]) {
                            acc[tableKey] = [];
                          }
                        ...
                        return Object.keys(groupedByTable).length > 0 ? (
                          Object.entries(groupedByTable).map(([tableNumber, items]) => (
                            <Card key={tableNumber} className="border-green-300 shadow-md">
                              ...
                              <CardContent className="space-y-3 pb-3 pt-3">
                                {items.map((item) => (
                                  <div key={item.id} className="flex items-center gap-3 bg-white p-3 rounded border border-green-200">
                                    <div className="flex-1">
                                      <div className="font-semibold text-base">{item.productName || "Unknown"}</div>
                                      <div className="text-sm text-gray-500">
                                        {item.orderNumber} - {item.orderedAt ?
                                    ...
                                    >
                                      ...
                                      <span className="text-xs">{t("kitchen.returnToProgress")}</span>
                                    </Button>
                                  </div>
                                ))}
                              </CardContent>
                      ...
                      (() => {
                        const groupedByDish = (completedItems || []).reduce((acc, item) => {
                          const dishKey = item.productName || "Unknown";
                          if (!acc[dishKey]) {
                            acc[dishKey] = [];
                          }
                        ...
                        return Object.keys(groupedByDish).length > 0 ? (
                          Object.entries(groupedByDish).map(([dishName, items]) => {
                            const totalQuantity = items.reduce((sum, item) => sum + (item.quantity || 0), 0);
                            return (
                              <Card key={dishName} className="border-green-300 shadow-md">
                                <CardHeader className="pb-3 bg-green-50">
                                  <div className="flex justify-between items-start">
                                    <div className="flex-1">
                                      <div className="font-bold text-lg">{dishName}</div>
                                ...
                                <CardContent className="space-y-3 pb-3 pt-3">
                                  {items.map((item) => (
                                    <div key={item.id} className="flex items-center gap-3 bg-white p-3 rounded border border-green-200">
                                      <div className="flex-1">
                                        <div className="font-semibold">{t("kitchen.table")} {item.tableNumber || "N/A"}</div>
                                        <div className="text-sm text-gray-500">
                                          {item.orderNumber} - {item.orderedAt ?
                                            new Date(item.orderedAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }) :
                                            ''}
