...
export function CategorySidebar({
  ...
  const { data: categoryCountsData } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/categories/product-counts"],
    queryFn: async () => {
      const response = await fetch(`https://api-pos.edpos.vn/api/categories/product-counts`);
  ...
  const handleBarcodeScan = () => {
    ...
    fetch(`https://api-pos.edpos.vn/api/products/barcode/${randomSku}`)
      .then((res) => res.json())
      .then((product) => {
        if (product.id) {
