...
export function PrinterConfigModal({
  ...
  const { data: printerConfigs = [], isLoading } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/printer-configs"],
    queryFn: async () => {
      const response = await apiRequest("GET", "https://api-pos.edpos.vn/api/printer-configs");
  ...
  const createConfigMutation = useMutation({
    mutationFn: async (configData: any) => {
      const response = await apiRequest(
        "POST",
        "https://api-pos.edpos.vn/api/printer-configs",
        configData,
      );
      return response.json();
    },
    onSuccess: () => {
      // Force refetch data
      queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/printer-configs"] });
      queryClient.refetchQueries({ queryKey: ["https://api-pos.edpos.vn/api/printer-configs"] });
      resetForm();
    },
    onError: () => {
      toast({
  ...
  const updateConfigMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const response = await apiRequest(
        "PUT",
        `https://api-pos.edpos.vn/api/printer-configs/${id}`,
        data,
      );
      return response.json();
    },
  ...
  const deleteConfigMutation = useMutation({
    mutationFn: async (id: number) => {
      await apiRequest("DELETE", `https://api-pos.edpos.vn/api/printer-configs/${id}`);
    },
    onSuccess: () => {
      // Force refetch data
      queryClient.invalidateQueries({ queryKey: ["https://api-pos.edpos.vn/api/printer-configs"] });
  ...
  const testConnectionMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest(
        "POST",
        `https://api-pos.edpos.vn/api/printer-configs/${id}/test`,
      );
      return response.json();
    },
    onSuccess: (data) => {
