...
export function CategoryStatsModal({ isOpen, onClose }: CategoryStatsModalProps) {
  ...
  const { data: productsData } = useQuery<any[]>({
    queryKey: ["http://42.118.102.26:4500/api/products", { includeInactive: true }],
    queryFn: async () => {
      const response = await fetch("http://42.118.102.26:4500/api/products?includeInactive=true");
