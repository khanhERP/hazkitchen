...
export default function ExpenseVoucherModal({
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("http://42.118.102.26:4500/api/customers");
  ...
  const { data: employees = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("http://42.118.102.26:4500/api/employees");
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        return Array.isArray(data) ? data : [];
      } catch (error) {
  ...
  const { data: suppliers = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("http://42.118.102.26:4500/api/suppliers");
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        return Array.isArray(data) ? data : [];
      } catch (error) {
  ...
  const createVoucherMutation = useMutation({
    mutationFn: async (data: ExpenseVoucher) => {
      ...
      const response = await fetch("http://42.118.102.26:4500/api/expense-vouchers", {
        method: "POST",
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["http://42.118.102.26:4500/api/expense-vouchers"] });
  ...
  const updateVoucherMutation = useMutation({
    mutationFn: async (data: ExpenseVoucher) => {
      ...
      const response = await fetch(`http://42.118.102.26:4500/api/expense-vouchers/${data.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  const deleteVoucherMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await fetch(`http://42.118.102.26:4500/api/expense-vouchers/${id}`, {
        method: "DELETE",
      });
