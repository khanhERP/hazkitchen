...
export function InvoiceManagementModal({
  ...
  const { data: invoices = [], isLoading } = useQuery<Invoice[]>({
    ...
    queryFn: async () => {
      const response = await apiRequest("GET", "https://api-pos.edpos.vn/api/invoices");
  ...
  const { data: invoiceItems = [] } = useQuery<InvoiceItem[]>({
    queryKey: ["https://api-pos.edpos.vn/api/invoice-items", selectedInvoice?.id],
    queryFn: async () => {
      if (!selectedInvoice?.id) return [];
      const response = await apiRequest("GET", `https://api-pos.edpos.vn/api/invoice-items/${selectedInvoice.id}`);
      return response.json();
    },
