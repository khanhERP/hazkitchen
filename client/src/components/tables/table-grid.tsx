...
export function TableGrid({ onTableSelect, selectedTableId }: TableGridProps) {
  ...
  const {
    ...
    queryKey: ["http://42.118.102.26:4500/api/order-items", selectedOrder?.id || "none"],
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest("GET", `http://42.118.102.26:4500/api/order-items/${orderId}`);
    ...
    const handleOrderUpdate = (event: CustomEvent) => {
      ...
      if (!event.detail?.skipAllRefetch && event.detail?.orderId) {
        queryClient.invalidateQueries({
          queryKey: ["http://42.118.102.26:4500/api/order-items", event.detail.orderId],
        });
      }
    };
  ...
  const updateTableStatusMutation = useMutation({
    mutationFn: ({ tableId, status }: { tableId: number; status: string }) =>
      apiRequest("PUT", `http://42.118.102.26:4500/api/tables/${tableId}/status`, { status }),
  ...
  const completePaymentMutation = useMutation({
    mutationFn: ({
      ...
      apiRequest("PUT", `http://42.118.102.26:4500/api/orders/${orderId}/status`, {
        status: "paid",
        paymentMethod,
      }),
    onSuccess: async (data, variables) => {
        ...
        try {
          ...
          if (otherActiveOrders.length === 0) {
            ...
            try {
              await apiRequest(
                "PUT",
                `http://42.118.102.26:4500/api/tables/${completedOrder.tableId}/status`,
                {
                  status: "available",
                },
              );
      ...
      try {
        ...
        const [freshTables, freshOrders] = await Promise.all([
          fetch("http://42.118.102.26:4500/api/tables", {
            cache: "no-store",
            headers: { "Cache-Control": "no-cache" },
          }).then((r) => r.json()),
          fetch("http://42.118.102.26:4500/api/orders", {
            cache: "no-store",
            headers: { "Cache-Control": "no-cache" },
          }).then((r) => r.json()),
        ]);
        ...
        queryClient.setQueryData(["http://42.118.102.26:4500/api/tables"], freshTables);
        queryClient.setQueryData(["http://42.118.102.26:4500/api/orders"], freshOrders);
      ...
      try {
        const [completedOrder, orderItemsData] = await Promise.all([
          queryClient.fetchQuery({
            queryKey: ["http://42.118.102.26:4500/api/orders", variables.orderId],
            queryFn: async () => {
              const response = await apiRequest(
                "GET",
                `http://42.118.102.26:4500/api/orders/${variables.orderId}`,
              );
              return response.json();
            },
          }),
          queryClient.fetchQuery({
            queryKey: ["http://42.118.102.26:4500/api/order-items", variables.orderId],
            queryFn: async () => {
              const response = await apiRequest(
                "GET",
                `http://42.118.102.26:4500/api/order-items/${variables.orderId}`,
              );
              return response.json();
            },
          }),
  ...
  const pointsPaymentMutation = useMutation({
    mutationFn: async ({
      ...
      // First redeem points
      await apiRequest("POST", "http://42.118.102.26:4500/api/customers/redeem-points", {
        customerId,
        points,
      });

      // Then mark order as paid
      await apiRequest("PUT", `http://42.118.102.26:4500/api/orders/${orderId}/status`, {
        status: "paid",
  ...
  const deleteOrderMutation = useMutation({
    mutationFn: async (orderId: number) => {
      ...
      if (order?.tableId) {
        ...
        if (!otherActiveOrders || otherActiveOrders.length === 0) {
          await apiRequest("PUT", `http://42.118.102.26:4500/api/tables/${order.tableId}/status`, {
            status: "available",
          });
        }
      }
    ...
    onSuccess: (data, orderId) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["http://42.118.102.26:4500/api/tables"] });
      queryClient.invalidateQueries({
        queryKey: ["http://42.118.102.26:4500/api/order-items", orderId],
      });
    },
    onError: () => {
      toast({
  ...
  const transferTableMutation = useMutation({
    mutationFn: async ({
      ...
      const orderUpdateResponse = await apiRequest(
        "PUT",
        `http://42.118.102.26:4500/api/orders/${orderId}`,
        {
          tableId: newTableId,
        },
      );
      ...
      try {
        await apiRequest("PUT", `http://42.118.102.26:4500/api/tables/${newTableId}/status`, {
          status: "occupied",
        });
      ...
      if (oldTableId) {
        try {
          ...
          const freshOrdersResponse = await apiRequest("GET", `http://42.118.102.26:4500/api/orders`);
          ...
          if (!otherActiveOrders || otherActiveOrders.length === 0) {
            await apiRequest("PUT", `http://42.118.102.26:4500/api/tables/${oldTableId}/status`, {
              status: "available",
            });
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        const [freshTables, freshOrders] = await Promise.all([
          fetch(`http://42.118.102.26:4500/api/tables?${params}`).then((r) => r.json()),
          fetch(`http://42.118.102.26:4500/api/orders?${params}`).then((r) => r.json()),
        ]);
  ...
  const recalculateOrderTotalMutation = useMutation({
    mutationFn: async (orderId: number) => {
      ...
      if (!remainingItems || remainingItems.length === 0) {
        ...
        const updateResult = await apiRequest("PUT", `http://42.118.102.26:4500/api/orders/${orderId}`, {
          subtotal: "0",
          tax: "0",
          total: "0",
        });
    ...
    onSuccess: (data, orderId) => {
      ...
      Promise.all([
        fetch("http://42.118.102.26:4500/api/orders", { cache: "no-store" }).then((r) => r.json()),
        fetch("http://42.118.102.26:4500/api/tables", { cache: "no-store" }).then((r) => r.json()),
        fetch(`http://42.118.102.26:4500/api/order-items/${orderId}`, { cache: "no-store" }).then((r) =>
          r.json(),
        ),
      ])
        .then(() => {
          ...
          queryClient.setQueryData(["http://42.118.102.26:4500/api/orders"], (oldData: any) => {
    ...
    try {
      ...
      if (currentOrderId) {
        try {
          const response = await apiRequest(
            "GET",
            `http://42.118.102.26:4500/api/order-items/${currentOrderId}`,
          );
  ...
  const handlePrintOrder = async (order: any) => {
    ...
    try {
      const orderItems = await queryClient.fetchQuery({
        queryKey: [`http://42.118.102.26:4500/api/order-items/${order.id}`],
        queryFn: async () => {
          const response = await apiRequest(
            "GET",
            `http://42.118.102.26:4500/api/order-items/${order.id}`,
      ...
      const response = await fetch("http://42.118.102.26:4500/api/auto-print", {
        method: "POST",
        headers: {
  ...
  const splitOrderMutation = useMutation({
    mutationFn: async (splitData: {
      ...
      const response = await apiRequest("POST", "http://42.118.102.26:4500/api/orders/split", {
      ...
      await Promise.all([
        ...
        refetchTables(),
        queryClient.invalidateQueries({
          queryKey: ["http://42.118.102.26:4500/api/order-items", splitData.originalOrderId],
        }),
      ]);
  ...
  const handleSplitOrder = async (splitData: any) => {
    try {
      ...
      const response = await apiRequest("POST", "http://42.118.102.26:4500/api/orders/split", splitData);
      ...
      <OrderDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open && editingOrder) {
            ...
            queryClient.invalidateQueries({ queryKey: ["http://42.118.102.26:4500/api/tables"] });
            queryClient.invalidateQueries({
              queryKey: ["http://42.118.102.26:4500/api/order-items", editingOrder.id],
            });
