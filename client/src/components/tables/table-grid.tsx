  ...
  const transferTableMutation = useMutation({
    mutationFn: async ({
      ...
      const orderUpdateResponse = await apiRequest(
        "PUT",
        `https://api-laundry-web.edpos.vn/api/orders/${orderId}`,
        {
          tableId: newTableId,
        },
      );
      ...
      try {
        await apiRequest("PUT", `https://api-laundry-web.edpos.vn/api/tables/${newTableId}/status`, {
          status: "occupied",
        });
      ...
      if (oldTableId) {
        try {
          ...
          const freshOrdersResponse = await apiRequest("GET", `https://api-laundry-web.edpos.vn/api/orders`);
          ...
          if (!otherActiveOrders || otherActiveOrders.length === 0) {
            await apiRequest("PUT", `https://api-laundry-web.edpos.vn/api/tables/${oldTableId}/status`, {
              status: "available",
            });
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        const [freshTables, freshOrders] = await Promise.all([
          fetch(`https://api-laundry-web.edpos.vn/api/tables?${params}`).then((r) => r.json()),
          fetch(`https://api-laundry-web.edpos.vn/api/orders?${params}`).then((r) => r.json()),
        ]);
