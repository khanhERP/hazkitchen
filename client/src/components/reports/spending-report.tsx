...
export function SpendingReport() {
  ...
  const {
    ...
    queryKey: ["https://api-pos.edpos.vn/api/purchase-receipts", { startDate, endDate }],
    queryFn: async () => {
      ...
      console.log("📊 Fetching purchase receipts with date filter:", {
        startDate,
        endDate,
        url: `https://api-pos.edpos.vn/api/purchase-receipts?${params.toString()}`,
      });

      const response = await fetch(
        `https://api-pos.edpos.vn/api/purchase-receipts?${params.toString()}`,
      );
  ...
  const { data: expenseVouchers = [], refetch: refetchExpenseVouchers } =
    useQuery({
      queryKey: ["https://api-pos.edpos.vn/api/expense-vouchers", { startDate, endDate }],
      queryFn: async () => {
        const params = new URLSearchParams();
        ...
        console.log("💰 Fetching expense vouchers with date filter:", {
          startDate,
          endDate,
          url: `https://api-pos.edpos.vn/api/expense-vouchers?${params.toString()}`,
        });

        const response = await fetch(
          `https://api-pos.edpos.vn/api/expense-vouchers?${params.toString()}`,
        );
  ...
  const { data: orders = [], refetch: refetchOrders } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/orders/date-range", startDate, endDate],
    queryFn: async () => {
      const response = await fetch(
        `https://api-pos.edpos.vn/api/orders/date-range/${startDate}/${endDate}/all`,
      );
