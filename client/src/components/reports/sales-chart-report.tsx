...
export function SalesChartReport() {
    ...
    queryFn: async () => {
      try {
        ...
        const response = await fetch(
          `https://api-pos.edpos.vn/api/orders/date-range/${startDateTimeISO}/${endDateTimeISO}${floorFilter}`,
        );
        if (!response.ok) {
  ...
  const { data: orderItems = [], isLoading: orderItemsLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://api-pos.edpos.vn/api/order-items");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://api-pos.edpos.vn/api/products/${selectedCategory}/${productType}/${productSearch || ""}`,
      );
  ...
  const { data: customers } = useQuery({
    queryKey: [
      "https://api-pos.edpos.vn/api/customers",
      customerSearch,
      customerStatus,
      startDate,
      endDate,
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://api-pos.edpos.vn/api/customers/${customerSearch || "all"}/${customerStatus}`,
      );
  ...
  const { data: productAnalysisData, isLoading: productAnalysisLoading } =
    useQuery({
      queryKey: [
        "https://api-pos.edpos.vn/api/product-analysis",
        startDate,
        endDate,
        startTime,
        endTime,
      ...
      queryFn: async () => {
        try {
          ...
          const response = await fetch(
            `https://api-pos.edpos.vn/api/product-analysis/${encodeURIComponent(startDateTimeLocal)}/${encodeURIComponent(endDateTimeLocal)}${floorFilter}?${params}`,
            {
              method: "GET",
              headers: {
  ...
  const { data: transactions } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/transactions"],
    staleTime: 5 * 60 * 1000,
  });
