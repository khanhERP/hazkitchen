...
export function OrderManagement() {
  ...
  const { data: orders, isLoading: ordersLoading } = useQuery({
    queryKey: ["http://42.118.102.26:4500/api/orders", "table"],
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        "http://42.118.102.26:4500/api/orders?salesChannel=table",
      );
      if (!response.ok) {
  ...
  const { data: orderItems, isLoading: orderItemsLoading } = useQuery({
    queryKey: ["http://42.118.102.26:4500/api/order-items", selectedOrder?.id],
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest(
          "GET",
          `http://42.118.102.26:4500/api/order-items/${selectedOrder.id}`,
        );
        if (!response.ok) {
          console.error(
  ...
  const updateOrderStatusMutation = useMutation({
    mutationFn: ({ orderId, status }: { orderId: number; status: string }) =>
      apiRequest("PUT", `http://42.118.102.26:4500/api/orders/${orderId}/status`, { status }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["http://42.118.102.26:4500/api/orders"] });
      queryClient.invalidateQueries({ queryKey: ["http://42.118.102.26:4500/api/tables"] });
    },
  ...
  const completeOrderPayment = async (orderId: number, paymentData: any) => {
    ...
    try {
      ...
      console.log("🔍 API Call Details:", {
        url: `http://42.118.102.26:4500/api/orders/${orderId}/status`,
        method: "PUT",
      ...
      const paymentDetailsResponse = await fetch(`http://42.118.102.26:4500/api/orders/${orderId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
