...
export function OrderDialog({
  ...
  const { data: existingOrderItems, refetch: refetchExistingItems } = useQuery({
    queryKey: ["https://api-pos.edpos.vn/api/order-items", existingOrder?.id],
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest(
        "GET",
        `https://api-pos.edpos.vn/api/order-items/${existingOrder.id}`,
      );
        ...
        if (mode === "edit" && existingOrder) {
          ...
          if (orderData.items.length > 0) {
            ...
            const addItemsResponse = await apiRequest(
              "POST",
              `https://api-pos.edpos.vn/api/orders/${existingOrder.id}/items`,
              {
          ...
          if (shouldRecalculate) {
            ...
            try {
              const recalcResponse = await apiRequest(
                "POST",
                `https://api-pos.edpos.vn/api/orders/${existingOrder.id}/recalculate`,
              );
          ...
          const updateResponse = await apiRequest(
            "PUT",
            `https://api-pos.edpos.vn/api/orders/${existingOrder.id}`,
            {
          ...
          const response = await apiRequest("POST", "https://api-pos.edpos.vn/api/orders", orderData);
    ...
    if (mode === "edit" && existingItem) {
      ...
      if (shouldSkipConfirmation) {
        ...
        if (newQuantity <= 0) {
          ...
          apiRequest("DELETE", `https://api-pos.edpos.vn/api/order-items/${existingItem.id}`)
            .then(async () => {
              console.log(
              ...
              queryClient.invalidateQueries({
                queryKey: ["https://api-pos.edpos.vn/api/order-items"],
              });
              queryClient.invalidateQueries({
          ...
          apiRequest("PUT", `https://api-pos.edpos.vn/api/order-items/${existingItem.id}`, {
          ...
          if (originalItem) {
            ...
            if (quantityChanged || discountChanged || unitPriceChanged) {
              try {
                ...
                await apiRequest(
                  "PUT",
                  `https://api-pos.edpos.vn/api/order-items/${currentItem.id}`,
                  updatePayload,
                );
  ...
  const handleConfirmReduceQty = async () => {
    ...
    try {
      await apiRequest("PUT", `https://api-pos.edpos.vn/api/order-items/${item.id}`, {
        quantity: newQuantity.toString(),
        notes: noteToSave,
      });
      console.log(
  ...
  const handleConfirmDeleteItem = async () => {
    ...
    try {
      await apiRequest("DELETE", `https://api-pos.edpos.vn/api/order-items/${item.id}`);
    ...
    if (isLastItem && existingOrder?.id) {
      ...
      try {
        // Cancel the order
        await apiRequest("PUT", `https://api-pos.edpos.vn/api/orders/${existingOrder.id}`, {
          status: "cancelled",
        });
        ...
        if (existingOrder.tableId) {
          await apiRequest(
            "PUT",
            `https://api-pos.edpos.vn/api/tables/${existingOrder.tableId}/status`,
            {
              status: "available",
            },
          );
