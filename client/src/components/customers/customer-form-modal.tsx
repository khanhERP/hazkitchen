...
export function CustomerFormModal({ isOpen, onClose, customer, initialPhone, onSuccess }: CustomerFormModalProps) {
  ...
  const { data: orderHistory, isLoading: ordersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest("GET", `https://api-pos.edpos.vn/api/orders`);
  ...
  const generateCustomerId = async () => {
    try {
      const response = await apiRequest("GET", "https://api-pos.edpos.vn/api/customers/next-id");
  ...
  const onSubmit = async (data: CustomerFormData) => {
    ...
    if (!customer?.id && (!customerIdToUse || customerIdToUse === t('common.autoGenerated'))) {
      ...
      try {
        const response = await fetch('https://api-pos.edpos.vn/api/customers/next-id');
