...
export function CustomerFormModal({ isOpen, onClose, customer, initialPhone }: CustomerFormModalProps) {
  ...
  const { data: orderHistory, isLoading: ordersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest("GET", `http://42.118.102.26:4500/api/orders`);
  ...
  const generateCustomerId = async () => {
    try {
      const response = await apiRequest("GET", "http://42.118.102.26:4500/api/customers/next-id");
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      const response = await apiRequest("POST", "http://42.118.102.26:4500/api/customers", data);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      ...
      const response = await apiRequest("PUT", `http://42.118.102.26:4500/api/customers/${customer.id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["http://42.118.102.26:4500/api/customers"] });
  ...
  const onSubmit = async (data: CustomerFormData) => {
    ...
    if (!customer?.id && (!customerIdToUse || customerIdToUse === t('common.autoGenerated'))) {
      ...
      try {
        const response = await fetch('http://42.118.102.26:4500/api/customers/next-id');
