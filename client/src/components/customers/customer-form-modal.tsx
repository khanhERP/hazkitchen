...
export function CustomerFormModal({ isOpen, onClose, customer, initialPhone }: CustomerFormModalProps) {
  ...
  const { data: orderHistory, isLoading: ordersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest("GET", `https://edpos-be.onrender.com/api/orders`);
  ...
  const generateCustomerId = async () => {
    try {
      const response = await apiRequest("GET", "https://edpos-be.onrender.com/api/customers/next-id");
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      const response = await apiRequest("POST", "https://edpos-be.onrender.com/api/customers", data);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      ...
      const response = await apiRequest("PUT", `https://edpos-be.onrender.com/api/customers/${customer.id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["https://edpos-be.onrender.com/api/customers"] });
  ...
  const onSubmit = async (data: CustomerFormData) => {
    ...
    if (!customer?.id && (!customerIdToUse || customerIdToUse === t('common.autoGenerated'))) {
      ...
      try {
        const response = await fetch('https://edpos-be.onrender.com/api/customers/next-id');
