...
export function CustomerFormModal({ isOpen, onClose, customer, initialPhone }: CustomerFormModalProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      const response = await apiRequest("POST", "https://api-laundry-web.edpos.vn/api/customers", data);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: CustomerFormData) => {
      ...
      const response = await apiRequest("PUT", `https://api-laundry-web.edpos.vn/api/customers/${customer.id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["https://api-laundry-web.edpos.vn/api/customers"] });
