...
export function PointsManagementModal({ isOpen, onClose }: PointsManagementModalProps) {
  ...
  const { data: pointTransactions, isLoading: transactionsLoading } = useQuery<PointTransaction[]>({
    queryKey: ['https://66622521-d7f0-4a33-aadd-c50d66665c71-00-wqfql649629t.pike.replit.dev/api/point-transactions'],
  ...
  const adjustPointsMutation = useMutation({
    mutationFn: async ({ customerId, points, type, description }: { 
      ...
      const response = await apiRequest('POST', 'https://66622521-d7f0-4a33-aadd-c50d66665c71-00-wqfql649629t.pike.replit.dev/api/customers/adjust-points', {
        customerId,
        points,
        type,
        description
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['https://66622521-d7f0-4a33-aadd-c50d66665c71-00-wqfql649629t.pike.replit.dev/api/point-transactions'] });
  ...
  const processPaymentMutation = useMutation({
    mutationFn: async ({ customerId, points }: { customerId: number; points: number }) => {
      const response = await apiRequest('POST', 'https://66622521-d7f0-4a33-aadd-c50d66665c71-00-wqfql649629t.pike.replit.dev/api/customers/redeem-points', {
        customerId,
        points
      });
      return response.json();
