import type { OrdersTranslations } from "../types";

export const ordersTranslations: { [key: string]: OrdersTranslations } = {
  ko: {
    orderManagement: "주문 관리",
    orderList: "주문 목록",
    placeOrder: "주문하기",
    salesOrderNumber: "판매 주문 번호",
    orderCreatedDate: "주문 생성일",
    orderNumberLabel: "주문 번호:",
    realTimeOrderStatus: "실시간 주문 현황을 확인하고 관리하세요",
    orderDetails: "주문 상세",
    orderNumber: "주문 번호",
    total: "총계",
    orderCode: "주문 코드",
    orderDate: "주문 날짜",
    orderSource: "주문 출처",
    customerCode: "고객 코드",
    customerName: "고객명",
    itemsOrdered: "주문 항목",
    orderInfo: "주문 정보",
    orderItems: "주문 항목",
    showing: "표시 중",
    item: "항목",
    orderStatus: "주문 상태",
    discount: "할인",
    orderTime: "주문 시간",
    orderStatusUpdated: "주문 상태 업데이트에 실패했습니다",
    orderStatusUpdateFailed: "주문 상태 업데이트에 실패했습니다",
    viewDetails: "세부 정보 보기",
    confirm: "확인",
    startCooking: "조리 시작",
    ready: "준비 완료",
    served: "서빙 완료",
    table: "테이블",
    customer: "고객",
    customerCount: "고객 수",
    noCustomerName: "고객명 없음",
    people: "명",
    unknownTable: "알 수 없는 테이블",
    unknownProduct: "알 수 없는 상품",
    memo: "메모",
    totalAmount: "총 금액",
    subtotal: "소계",
    tax: "세금",
    statusAndTime: "상태 및 시간",
    customerName: "고객명",
    ordersInProgress: "개 진행중",
    noActiveOrders: "진행중인 주문이 없습니다",
    newOrdersWillAppearHere: "새로운 주문이 들어오면 여기에 표시됩니다",
    selectPaymentMethod: "결제 방법 선택",
    selectPaymentMethodDesc: "주문에 대한 결제 방법을 선택하세요",
    cancel: "취소",
    cancelOrder: "주문 취소",
    bulkCancelTitle: "판매 주문 취소",
    bulkCancelDescription: "선택한 {count}개의 주문을 취소하시겠습니까? 이 작업은 취소할 수 없습니다.",
    bulkCancelConfirm: "{count}개 주문 취소",
    bulkCancelCancel: "취소",
    confirmCancelOrderTitle: "주문 취소 확인",
    confirmCancelOrderDescription: "주문 {orderNumber}를 취소하시겠습니까? 이 작업은 취소할 수 없습니다.",
    confirmCancelButton: "취소 확인",
    pointsPaymentTitle: "포인트로 결제",
    pointsPaymentDesc: "고객을 선택하고 포인트를 사용하여 주문 결제",
    searchCustomers: "고객 검색",
    availablePoints: "사용 가능한 포인트",
    customerNotFound: "고객을 찾을 수 없음",
    mixedPaymentTitle: "혼합 결제",
    mixedPaymentButton: "혼합 결제",
    mixedPaymentCancel: "취소",
    mixedPaymentCompleted: "혼합 결제 완료",
    mixedPaymentError: "혼합 결제 오류",
    mixedPaymentDesc:
      "포인트가 부족하여 현금 또는 이체로 추가 결제가 필요합니다",
    paymentSummary: "결제 요약",
    mixedPaymentSummary: "결제 요약",
    printBill: "청구서 인쇄",
    totalOrder: "주문 총액",
    pointsPayment: "포인트 결제",
    remainingAmount: "남은 금액",
    addMore: "추가 주문",
    editOrder: "주문 수정",
    editOrderTitle: "주문 수정 - 테이블",
    editOrderDesc: "주문 {orderNumber} | 메뉴 추가 또는 수량 수정",
    addNewItems: "새 메뉴 추가",
    previouslyOrdered: "이전 주문 메뉴:",
    alreadyOrdered: "주문됨",
    newItemsToAdd: "새로 추가할 메뉴:",
    itemsAndNewItems: "주문 메뉴 & 새 메뉴",
    previousItems: "이전 메뉴",
    newItems: "새 메뉴 추가",
    updateOrder: "주문 업데이트",
    updating: "업데이트 중...",
    orderUpdateSuccess: "주문 업데이트",
    orderUpdateSuccessDesc: "주문에 새 메뉴가 성공적으로 추가되었습니다",
    updateAndRefresh: "업데이트 및 새로고침",
    showingItems: "{{count}}개 항목 표시 중 - 주문 {{orderNumber}}",
    use: "사용",
    enoughpointstopay: "전체 주문 결제에 충분한 포인트",

    pointsPaymentDialog: {
      title: "포인트 결제",
      description: "고객을 선택하고 포인트로 주문을 결제하세요",
      orderInfo: "주문 정보",
      orderCode: "주문 코드:",
      totalAmount: "총액:",
      searchCustomer: "고객 검색",
      searchPlaceholder: "이름, 전화번호 또는 고객 코드로 검색...",
      accumulatedPoints: "적립 포인트",
      mixedPayment: "혼합 결제",
    },
    paymentMethods: {
      cash: "현금",
      creditCard: "신용카드",
      debitCard: "체크카드",
      momo: "모모",
      zalopay: "잘로페이",
      vnpay: "VN페이",
      qrCode: "QR 코드",
      shopeepay: "쇼피페이",
      grabpay: "그랩페이",
    },
    noTableInfo: "테이블 정보 없음",
    noInput: "미입력",
    payment: "결제",
    orderPlaced: "주문 완료",
    orderPlacedSuccess: "주문이 성공적으로 접수되었습니다",
    orderFailed: "주문 접수에 실패했습니다",
    tableOrder: "테이블 주문",
    tableCapacity: "테이블 정원",
    status: {
      pending: "대기중",
      confirmed: "확인됨",
      preparing: "조리중",
      ready: "준비완료",
      served: "서빙완료",
      paid: "결제완료",
      cancelled: "취소됨",
      delivering: "배송중",
      completed: "완료됨",
      ordered: "주문됨",
    },
    orderCount: "주문",
    displaying: "표시 중",
    items: "항목",
    quantity: "수량",
    remaining: "남은",
    notenoughpoints: "포인트가 부족하여 혼합 결제가 필요합니다",
    useall: "모두 사용",
    additionalpaymentrequired: "추가 결제 필요",
    selectpaymentmethod: "남은 금액에 대한 결제 방법을 선택하세요",
    productSearch: "상품 검색",
    productSearchPlaceholder: "상품명 또는 코드로 검색",
    searchPlaceholder: "이름 또는 SKU로 검색...",
    itemTotal: "항목 총액",
    salesChannelAll: "전체",
    salesChannelTable: "매장 식사",
    salesChannelPos: "카운터 판매",
    salesChannelOnline: "온라인 판매",
    salesChannelDelivery: "배달",
    orderStatusAll: "전체",
    orderStatusPaid: "결제 완료",
    orderStatusPending: "처리 대기",
    orderStatusCancelled: "취소됨",
    einvoiceStatusAll: "전체",
    einvoiceStatusNotPublished: "미발행",
    einvoiceStatusPublished: "발행됨",
    statusLabel: "상태",
    invoiceStatusLabel: "송장 상태",
    einvoiceStatusLabel: "전자세금계산서 상태",
    cancelCompletionDate: "취소/완료 날짜",
    salesOrderNumber: "판매 주문 번호",
    invoiceSymbol: "송장 기호",
    date: "날짜",
    orderDate: "날짜",
    phone: "전화번호",
    itemList: "상품 목록",
    summary: "요약",
    totalPayment: "총 결제 금액",
    priceBeforeTax: "세전 금액",
    publishInvoice: "송장 발행",
    printInvoice: "송장 인쇄",
    close: "닫기",
    memo: "메모",
    notesPlaceholder: "메모 입력...",
    confirmDeleteItem: "\"{itemName}\"을(를) 주문에서 삭제하시겠습니까?",

    // Order change history
    orderChangeHistory: "주문 변경 이력",
    fromDate: "시작일",
    toDate: "종료일",
    orderNumberFilter: "주문 번호",
    orderNumberPlaceholder: "주문 번호 입력",
    userNameFilter: "사용자",
    userNamePlaceholder: "사용자 이름 입력",
    actionFilter: "작업 유형",
    allActions: "전체",
    searchInDetails: "세부정보에서 검색",
    searchPlaceholder: "검색...",
    recordsPerPage: "페이지당 레코드",
    timeColumn: "시간",
    orderCodeColumn: "주문 코드",
    userColumn: "사용자",
    actionColumn: "작업 유형",
    detailsColumn: "변경 세부정보",
    ipAddressColumn: "IP 주소",
    showing: "표시 중",
    of: "총",
    records: "레코드",
    recordsPerPage: "페이지당 레코드",

    // Action types
    actions: {
      increase_quantity: "수량 증가",
      decrease_quantity: "수량 감소",
      update_quantity: "수량 업데이트",
      update_discount: "할인 업데이트",
      delete_item: "항목 삭제",
      add_item: "항목 추가",
      update_order: "주문 업데이트",
    },
  },
  en: {
    orderManagement: "Order Management",
    orderList: "Order List",
    placeOrder: "Place Order",
    orderDetails: "Order Details",
    orderNumber: "Order Number",
    salesOrderNumber: "Sales Order Number",
    orderCreatedDate: "Order Created Date",
    total: "Total",
    orderCode: "Order Code",
    orderDate: "Order Date",
    orderSource: "Order Source",
    customerCode: "Customer Code",
    customerName: "Customer Name",
    item: "Item",
    itemsOrdered: "Items Ordered",
    orderInfo: "Order Information",
    orderItems: "Order Items",
    showing: "Showing",
    discount: "Discount",
    orderNotes: "Order Notes",
    showingItems: "Showing {{count}} items - Order {{orderNumber}}",
    orderStatus: "Order Status",
    orderTime: "Order Time",
    orderStatusUpdated: "Order status updated successfully",
    orderStatusUpdateFailed: "Failed to update order status",
    viewDetails: "View Details",
    confirm: "Confirm",
    startCooking: "Start Cooking",
    ready: "Ready",
    served: "Served",
    table: "Table",
    customer: "Customer",
    customerCount: "Customer Count",
    noCustomerName: "No customer name",
    people: " people",
    unknownTable: "Unknown Table",
    unknownProduct: "Unknown Product",
    memo: "Memo",
    totalAmount: "Total Amount",
    subtotal: "Subtotal",
    tax: "Tax",
    statusAndTime: "Status & Time",
    realTimeOrderStatus: "Check and manage real-time order status",
    ordersInProgress: " orders in progress",
    noActiveOrders: "No active orders",
    newOrdersWillAppearHere: "New orders will appear here",
    selectPaymentMethod: "Select Payment Method",
    selectPaymentMethodDesc: "Choose payment method for the order",
    cancel: "Cancel",
    cancelOrder: "Cancel Order",
    bulkCancelTitle: "Cancel Sales Orders",
    bulkCancelDescription: "Are you sure you want to cancel {count} selected order(s)? This action cannot be undone.",
    bulkCancelConfirm: "Cancel {count} Order(s)",
    bulkCancelCancel: "Cancel",
    confirmCancelOrderTitle: "Confirm Cancel Order",
    confirmCancelOrderDescription: "Are you sure you want to cancel order {orderNumber}? This action cannot be undone.",
    confirmCancelButton: "Confirm Cancel",
    pointsPaymentTitle: "Pay with Points",
    pointsPaymentDesc: "Select customer and points to pay for order",
    searchCustomers: "Search customers",
    availablePoints: "Available points",
    customerNotFound: "Customer not found",
    searchCustomerPlaceholder: "Search by name, phone, or customer code...",
    mixedPaymentTitle: "Mixed Payment",
    mixedPaymentButton: "Mixed Payment",
    customerName: "Customer Name",
    mixedPaymentCancel: "Cancel Mixed Payment",
    mixedPaymentCompleted: "Mixed Payment Completed",
    mixedPaymentError: "Mixed Payment Error",
    mixedPaymentDesc:
      "Insufficient points, need additional payment by cash or transfer",
    paymentSummary: "Payment Summary",
    mixedPaymentSummary: "Payment Summary",
    printBill: "Print Bill",
    totalOrder: "Order Total",
    pointsPayment: "Points Payment",
    remainingAmount: "Remaining Amount",
    addMore: "Add More",
    editOrder: "Edit Order",
    editOrderTitle: "Edit Order - Table",
    editOrderDesc: "Order {orderNumber} | Add items or edit quantity",
    addNewItems: "Add New Items",
    previouslyOrdered: "Previously ordered:",
    alreadyOrdered: "Ordered",
    newItemsToAdd: "New items to add:",
    itemsAndNewItems: "Ordered Items & New Items",
    previousItems: "Previous Items",
    newItems: "New Items",
    updateOrder: "Update Order",
    updating: "Updating...",
    orderUpdateSuccess: "Order Updated",
    orderUpdateSuccessDesc:
      "New items have been successfully added to the order",
    updateAndRefresh: "Update & Refresh",
    pointsPaymentDialog: {
      title: "Points Payment",
      description: "Select customer and use points to pay for the order",
      orderInfo: "Order Information",
      orderCode: "Order Code:",
      totalAmount: "Total Amount:",
      searchCustomer: "Search Customer",
      searchPlaceholder: "Search by name, phone, or customer code...",
      accumulatedPoints: "Accumulated Points",
      mixedPayment: "Mixed Payment",
    },
    paymentMethods: {
      cash: "Cash",
      creditCard: "Credit Card",
      debitCard: "Debit Card",
      momo: "MoMo",
      zalopay: "ZaloPay",
      vnpay: "VNPay",
      qrCode: "QR Code",
      shopeepay: "ShopeePay",
      grabpay: "GrabPay",
    },
    noTableInfo: "No table information",
    noInput: "Not entered",
    payment: "Payment",
    orderPlaced: "Order Placed",
    orderPlacedSuccess: "Order has been successfully placed",
    orderFailed: "Failed to place order",
    tableOrder: "Table Order",
    tableCapacity: "Table Capacity",
    status: {
      pending: "Pending",
      confirmed: "Confirmed",
      preparing: "Preparing",
      ready: "Ready",
      served: "Served",
      paid: "Paid",
      cancelled: "Cancelled",
      delivering: "Delivering",
      completed: "Completed",
      ordered: "Ordered",
    },
    orderCount: "orders",
    displaying: "Displaying",
    items: "items",
    use: "Use",
    quantity: "Quantity",
    enoughpointstopay: "Enough points to pay for the entire order",
    remaining: "Remaining",
    notenoughpoints: "Not enough points, need mixed payment",
    useall: "Use All",
    additionalpaymentrequired: "Additional payment required",
    selectpaymentmethod: "Select payment method for the remaining amount",
    productSearch: "Product Search",
    productSearchPlaceholder: "Search by product name or code",
    searchPlaceholder: "Search by name or SKU...",
    itemTotal: "Item Total",
    salesChannelAll: "All",
    salesChannelTable: "Dine In",
    salesChannelPos: "Counter Sales",
    salesChannelOnline: "Online Sales",
    salesChannelDelivery: "Delivery",
    orderStatusAll: "All",
    orderStatusPaid: "Paid",
    orderStatusPending: "Pending",
    orderStatusCancelled: "Cancelled",
    einvoiceStatusAll: "All",
    einvoiceStatusNotPublished: "Not Published",
    einvoiceStatusPublished: "Published",
    statusLabel: "Status",
    invoiceStatusLabel: "Invoice Status",
    einvoiceStatusLabel: "E-Invoice Status",
    cancelCompletionDate: "Cancellation/Completion Date",
    salesOrderNumber: "Sales Order Number",
    invoiceSymbol: "Invoice Symbol",
    date: "Date",
    orderDate: "Date",
    phone: "Phone",
    itemList: "Item List",
    summary: "Summary",
    totalPayment: "Total Payment",
    priceBeforeTax: "Price Before Tax",
    publishInvoice: "Publish Invoice",
    printInvoice: "Print Invoice",
    close: "Close",
    memo: "Memo",
    notesPlaceholder: "Enter notes...",
    confirmDeleteItem: "Are you sure you want to remove \"{itemName}\" from the order?",

    // Order change history
    orderChangeHistory: "Order Change History",
    fromDate: "From Date",
    toDate: "To Date",
    orderNumberFilter: "Order Number",
    orderNumberPlaceholder: "Enter order number",
    userNameFilter: "User Name",
    userNamePlaceholder: "Enter user name",
    actionFilter: "Action Type",
    allActions: "All",
    searchInDetails: "Search in Details",
    searchPlaceholder: "Search...",
    recordsPerPage: "records/page",
    timeColumn: "Time",
    orderCodeColumn: "Order Code",
    userColumn: "User",
    actionColumn: "Action Type",
    detailsColumn: "Change Details",
    ipAddressColumn: "IP Address",
    showing: "Showing",
    of: "of",
    records: "records",
    recordsPerPage: "records/page",

    // Action types
    actions: {
      increase_quantity: "Increase Quantity",
      decrease_quantity: "Decrease Quantity",
      update_quantity: "Update Quantity",
      update_discount: "Update Discount",
      delete_item: "Delete Item",
      add_item: "Add Item",
      update_order: "Update Order",
    },
  },
  vi: {
    orderManagement: "Danh sách đơn hàng bán",
    salesOrderNumber: "Số đơn bán",
    orderCreatedDate: "Ngày tạo đơn",
    orderList: "Danh sách đơn hàng",
    orderDetails: "Chi tiết đơn hàng",
    placeOrder: "Đặt hàng",
    orderNumber: "Số đơn hàng",
    orderCode: "Số đơn bán",
    orderDate: "Ngày đơn bán",
    orderSource: "Nguồn đơn",
    itemsOrdered: "Món đã đặt",
    orderInfo: "Thông tin đơn hàng",
    orderItems: "Các món đã đặt",
    showing: "Hiển thị",
    total: "Tổng cộng",
    customerCode: "Mã khách hàng",
    customerName: "Tên khách hàng",
    orderNotes: "Ghi chú đơn hàng",
    discount: "Giảm giá",
    item: "Món",
    showingItems: "Hiển thị {{count}} món - Đơn hàng {{orderNumber}}",
    orderStatus: "Trạng thái đơn hàng",
    orderTime: "Thời gian đặt",
    orderStatusUpdated: "Cập nhật trạng thái đơn hàng thành công",
    orderStatusUpdateFailed: "Cập nhật trạng thái đơn hàng thất bại",
    viewDetails: "Xem chi tiết",
    confirm: "Xác nhận",
    startCooking: "Bắt đầu nấu",
    ready: "Sẵn sàng",
    served: "Đang phục vụ",
    table: "Bàn",
    customer: "Khách hàng",
    customerCount: "Số khách",
    noCustomerName: "Chưa có tên khách",
    people: " người",
    unknownTable: "Bàn không xác định",
    unknownProduct: "Sản phẩm không xác định",
    memo: "Ghi chú",
    totalAmount: "Tổng tiền",
    subtotal: "Tổng phụ",
    tax: "Thuế",
    statusAndTime: "Trạng thái & Thời gian",
    realTimeOrderStatus:
      "Kiểm tra và quản lý tình trạng đơn hàng theo thời gian thực",
    ordersInProgress: "đơn đang xử lý",
    noActiveOrders: "Không có đơn hàng đang hoạt động",
    newOrdersWillAppearHere: "Đơn hàng mới sẽ xuất hiện ở đây",
    selectPaymentMethod: "Chọn phương thức thanh toán",
    selectPaymentMethodDesc: "Chọn phương thức thanh toán cho đơn hàng",
    cancel: "Hủy",
    cancelOrder: "Hủy đơn",
    bulkCancelTitle: "Hủy đơn hàng bán",
    bulkCancelDescription: "Bạn có chắc muốn hủy {count} đơn hàng đã chọn không? Hành động này không thể hoàn tác.",
    bulkCancelConfirm: "Hủy {count} đơn",
    bulkCancelCancel: "Bỏ qua",
    confirmCancelOrderTitle: "Xác nhận hủy đơn hàng",
    confirmCancelOrderDescription: "Bạn có chắc chắn muốn hủy đơn hàng {orderNumber} này không? Hành động này không thể hoàn tác.",
    confirmCancelButton: "Xác nhận hủy",
    pointsPaymentTitle: "Thanh toán bằng điểm",
    pointsPaymentDesc: "Chọn khách hàng và số điểm để thanh toán đơn hàng",
    searchCustomers: "Tìm kiếm khách hàng",
    availablePoints: "Điểm có sẵn",
    customerNotFound: "Không tìm thấy khách hàng",
    searchCustomerPlaceholder:
      "Tìm theo tên, số điện thoại hoặc mã khách hàng...",
    mixedPaymentTitle: "Thanh toán hỗn hợp",
    mixedPaymentButton: "Thanh toán hỗn hợp",
    mixedPaymentCancel: "Hủy thanh toán hỗn hợp",
    mixedPaymentCompleted: "Thanh toán hỗn hợp hoàn tất",
    mixedPaymentError: "Lỗi thanh toán hỗn hợp",
    mixedPaymentDesc:
      "Không đủ điểm, cần thanh toán thêm bằng tiền mặt hoặc chuyển khoản",
    paymentSummary: "Tóm tắt thanh toán",
    mixedPaymentSummary: "Tóm tắt thanh toán",
    printBill: "In hóa đơn",
    use: "Sử dụng",
    totalOrder: "Tổng đơn hàng",
    pointsPayment: "Thanh toán bằng điểm",
    remainingAmount: "Cần thanh toán thêm",
    addMore: "Gọi thêm",
    editOrder: "Chỉnh sửa đơn hàng",
    editOrderTitle: "Chỉnh sửa đơn hàng - Bàn",
    editOrderDesc: "Đơn hàng {orderNumber} | Thêm món hoặc chỉnh sửa số lượng",
    addNewItems: "Thêm món mới",
    previouslyOrdered: "Món đã gọi trước đó:",
    alreadyOrdered: "Đã gọi",
    newItemsToAdd: "Món mới thêm:",
    itemsAndNewItems: "Món đã gọi & Món mới",
    previousItems: "Món đã gọi",
    newItems: "Món mới thêm",
    updateOrder: "Cập nhật đơn hàng",
    updating: "Đang cập nhật...",
    orderUpdateSuccess: "Cập nhật đơn hàng",
    orderUpdateSuccessDesc: "Đơn hàng đã được cập nhật thành công",
    updateAndRefresh: "Cập nhật & Làm mới",
    pointsPaymentDialog: {
      title: "Thanh toán bằng điểm",
      description: "Chọn khách hàng và số điểm để thanh toán đơn hàng",
      orderInfo: "Thông tin đơn hàng",
      orderCode: "Mã đơn:",
      totalAmount: "Tổng tiền:",
      searchCustomer: "Tìm kiếm khách hàng",
      searchPlaceholder: "Tìm theo tên, số điện thoại hoặc mã khách hàng...",
      accumulatedPoints: "Điểm tích lũy",
      mixedPayment: "Thanh toán hỗn hợp",
    },
    paymentMethods: {
      cash: "Tiền mặt",
      creditCard: "Thẻ tín dụng",
      debitCard: "Thẻ ghi nợ",
      momo: "MoMo",
      zalopay: "ZaloPay",
      vnpay: "VNPay",
      qrCode: "QR Code",
      shopeepay: "ShopeePay",
      grabpay: "GrabPay",
    },
    noTableInfo: "Không có thông tin bàn",
    noInput: "Chưa nhập",
    payment: "Thanh toán",
    orderPlaced: "Đặt hàng",
    orderPlacedSuccess: "Đơn hàng đã được đặt thành công",
    orderFailed: "Đặt hàng thất bại",
    tableOrder: "Đặt hàng bàn",
    tableCapacity: "Số Người",
    status: {
      pending: "Chờ xử lý",
      confirmed: "Đã xác nhận",
      preparing: "Đang chuẩn bị",
      ready: "Sẵn sàng",
      served: "Đang phục vụ",
      paid: "Đã thanh toán",
      cancelled: "Đã hủy",
      delivering: "Đang giao hàng",
      completed: "Hoàn thành",
      ordered: "Đặt hàng",
    },
    orderCount: "đơn",
    displaying: "Hiển thị",
    items: "món",
    quantity: "Số lượng",
    enoughpointstopay: "Đủ điểm để thanh toán toàn bộ đơn hàng",
    remaining: "Còn lại",
    notenoughpoints: "Không đủ điểm, cần thanh toán hỗn hợp",
    useall: "Sử dụng tất cả",
    additionalpaymentrequired: "Cần thanh toán thêm",
    selectpaymentmethod: "Chọn phương thức thanh toán cho phần còn lại",
    productSearch: "Mặt hàng",
    productSearchPlaceholder: "Tìm theo tên hoặc mã mặt hàng",
    searchPlaceholder: "Tìm kiếm theo tên hoặc SKU...",
    itemTotal: "Thành tiền",
    salesChannelAll: "Tất cả",
    salesChannelTable: "Ăn tại chỗ",
    salesChannelPos: "Bán tại quầy",
    salesChannelOnline: "Bán online",
    salesChannelDelivery: "Giao hàng",
    orderStatusAll: "Tất cả",
    orderStatusPaid: "Đã thanh toán",
    orderStatusPending: "Chờ xử lý",
    orderStatusCancelled: "Đã hủy",
    einvoiceStatusAll: "Tất cả",
    einvoiceStatusNotPublished: "Chưa phát hành",
    einvoiceStatusPublished: "Đã phát hành",
    statusLabel: "Trạng thái",
    invoiceStatusLabel: "Trạng thái hóa đơn",
    einvoiceStatusLabel: "Trạng thái hóa đơn điện tử",
    cancelCompletionDate: "Ngày hủy đơn/hoàn thành",
    salesOrderNumber: "Số đơn bán",
    invoiceSymbol: "Ký hiệu hóa đơn",
    date: "Ngày",
    orderDate: "Ngày",
    phone: "Điện thoại",
    itemList: "Danh sách hàng hóa",
    summary: "Tóm tắt",
    totalPayment: "Tổng tiền thanh toán",
    priceBeforeTax: "Tiền trước thuế",
    publishInvoice: "Phát hành hóa đơn",
    printInvoice: "In hóa đơn",
    close: "Đóng",
    memo: "Ghi chú",
    notesPlaceholder: "Nhập ghi chú...",
    confirmDeleteItem: "Bạn có chắc chắn muốn xóa \"{itemName}\" khỏi đơn hàng?",

    // Order change history
    orderChangeHistory: "Lịch sử thao tác đơn hàng",
    fromDate: "Từ ngày",
    toDate: "Đến ngày",
    orderNumberFilter: "Mã đơn hàng",
    orderNumberPlaceholder: "Nhập số đơn hàng",
    userNameFilter: "Người dùng",
    userNamePlaceholder: "Nhập tên người dùng",
    actionFilter: "Loại thao tác",
    allActions: "Tất cả",
    searchInDetails: "Tìm trong chi tiết",
    searchPlaceholder: "Tìm kiếm...",
    recordsPerPage: "bản ghi/trang",
    timeColumn: "Thời gian",
    orderCodeColumn: "Mã đơn",
    userColumn: "Người thao tác",
    actionColumn: "Loại thao tác",
    detailsColumn: "Chi tiết thay đổi",
    ipAddressColumn: "IP Address",
    showing: "Hiển thị",
    of: "trong tổng",
    records: "bản ghi",
    recordsPerPage: "bản ghi/trang",

    // Action types
    actions: {
      increase_quantity: "Tăng SL",
      decrease_quantity: "Giảm SL",
      update_quantity: "Cập nhật SL",
      update_discount: "Cập nhật giảm giá",
      delete_item: "Xóa",
      add_item: "Thêm",
      update_order: "Cập nhật đơn",
    },
  },
};